...
export const runComparison=(csrfToken:string,baselinePath:string,targetPath:string,baselineLabel:string,targetLabel:string)=>requestJson<ComparisonResponse>('/api/compare',{method:'POST',headers:csrfHeaders(csrfToken),body:JSON.stringify({baseline_path:baselinePath,target_path:targetPath,baseline_label:baselineLabel,target_label:targetLabel,report_formats:['json','md','html']})});
